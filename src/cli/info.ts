...
const REGISTRY_LATEST = "https://registry.npmjs.org/memoo-mcp/latest";
...
export function printHelp(write: (s: string) => void = console.log): void {
  write(`Usage: memoo-mcp [command] [options]
...
Environment (stdio):
  MEMOO_API_KEY            Required API key (moo_sk…)
  MEMOO_NAMESPACE          Required namespace UUID or name
  MEMOO_BASE_URL           Upstream API (default https://memoo.hinha.web.id)
  MEMOO_TIMEOUT            Timeout (e.g. 600s, 5m)

Examples:
  npx -y memoo-mcp@latest --version
  memoo-mcp update
  MEMOO_API_KEY=moo_sk_… MEMOO_NAMESPACE=my-ns memoo-mcp --stdio
  memoo-mcp serve
`);
}
  ...
  try {
    ...
    if (compareSemver(currentVersion, latest) >= 0) {
      return {
        ...
        latest,
        status: "up_to_date",
        message: `memoo-mcp is up to date (${currentVersion})`,
      };
    }
    return {
      current: currentVersion,
      ...
      message: [
        ...
        "Update with: npm i -g memoo-mcp@latest",
        "Or use: npx -y memoo-mcp@latest",
      ].join("\n"),
    };
  } catch (err) {
